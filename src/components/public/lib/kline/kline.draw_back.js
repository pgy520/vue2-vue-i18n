...
GLOBAL_VAR.periodMap = {
    "01w" : "1week" , "03d" : "3day", "01d" : "1day" , "12h" : "12hour",
...
Chart.strPeriod = {
    'zh-cn': {'line':'(分时)','1min':'(1分钟)','5min':'(5分钟)','15min':'(15分钟)','30min':'(30分钟)','1hour':'(1小时)','1day':'(日线)','1week':'(周线)'
        ,'3min':'(3分钟)', '2hour':'(2小时)','4hour':'(4小时)','6hour':'(6小时)','12hour':'(12小时)','3day':'(3天)'},
    'en-us': {'line':'(Line)','1min':'(1m)' ,'5min':'(5m)'   ,'15min':'(15m)'   ,'30min':'(30m)'   ,'1hour':'(1h)'   ,'1day':'(1d)'  ,'1week':'(1w)'
        ,'3min':'(3m)', '2hour':'(2h)','4hour':'(4h)','6hour':'(6h)','12hour':'(12h)','3day':'(3d)'},
    'zh-tw': {'line':'(分時)','1min':'(1分钟)','5min':'(5分钟)','15min':'(15分钟)','30min':'(30分钟)','1hour':'(1小時)','1day':'(日线)','1week':'(周线)'
        ,'3min':'(3分钟)', '2hour':'(2小時)','4hour':'(4小時)','6hour':'(6小時)','12hour':'(12小時)','3day':'(3天)'}
};
...
Chart.prototype.setMarketFrom = function(type) {
...
Chart.prototype.updateDataAndDisplay = function() {
    ...
    if (f == -1) {
        ...
        GLOBAL_VAR.requestParam = setHttpRequestParam(GLOBAL_VAR.market_from, GLOBAL_VAR.time_type, null, f.toString());
        RequestData();
    }
 ...
 * @zhanglinbo
 */
ChartManager.prototype.getLastTime =function(dsAlias){
	   var cached = this.getCachedDataSource(dsAlias);
	    if (cached != null) {
	    	var count = cached._dataItems.length;
	        if (count < 1)
	            return -1;
	        return cached._dataItems[count - 1].date;
	    }else{
	    	return -1;
	    }
};
...
DarkTheme.prototype.__construct = function() {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#e83057";
    this._colors[Theme.Color.Negative] = "#31c871";
    this._colors[Theme.Color.PositiveDark] = "#e83057";
    this._colors[Theme.Color.NegativeDark] = "#31c871";
    this._colors[Theme.Color.Unchanged] = "#fff";
    this._colors[Theme.Color.Background] = "#292d37";
...
LightTheme.prototype.__construct = function() {
    ...
    this._colors[Theme.Color.Grid0] = "#fff";
    this._colors[Theme.Color.Grid1] = "#EBEBEB";
...
ChartSettings.init = function() {
    ...
    _frame.indicsStatus = 'off';
    _frame.toolStatus = 'off';
    _frame.period = _time;
    ChartSettings._data = {
        ver: 1,
        charts: _frame,
...
ChartSettings.save = function () {
    ...
    document.cookie = "chartSettings=" + decodeURIComponent(JSON.stringify(ChartSettings._data))
...
var RequestData = function(showLoading) {
    ...
    if(!klineInterVal){
        klineInterVal = setInterval(function(){
            //if(top.ajaxRun){
...
function updateKlineData(json){
	var channel = kline.symbol.replace('_','')+"_"+ (kline.money || kline.symbol.split('_')[1]) +"_kline_"+GLOBAL_VAR.time_type;
	if(channel!=json.channel){
		//console.log(json.channel+"---"+channel+" channel不同不处理");
		return;
	}
	 GLOBAL_VAR.market_from_name=json.datas.symbol;
    ...
    /* if(GLOBAL_VAR.KLineData.length>0){
    	 var lastObj =GLOBAL_VAR.KLineData[GLOBAL_VAR.KLineData.length-1];
     	// console.log(">>>："+json.datas.data.length);
     	 if(json.datas.data.length==1){
     		 //console.log(lastObj[0]+"---"+json.datas.data[0][0]);
     		 if(lastObj[0]==json.datas.data[0][0]){
     			
         		 //console.log("移除前："+GLOBAL_VAR.KLineData.length);
         		var obj= Array.prototype.pop.apply(GLOBAL_VAR.KLineData);
         		// console.log("移除后： "+GLOBAL_VAR.KLineData.length);
         		if(obj){
         			Array.prototype.push.apply(GLOBAL_VAR.KLineData,json.datas.data);
         		}
         	 }else{
         		// console.log("时间不相等："+json.datas.data.length);
         		 Array.prototype.push.apply(GLOBAL_VAR.KLineData,json.datas.data);
         	 }
     		//console.log("1. "+GLOBAL_VAR.KLineData.length+" ---> "+json.datas.data.length);
          }else{
         	 
         	// console.log("前："+GLOBAL_VAR.KLineData.length);
         	 ...
         	 //console.log("后："+GLOBAL_VAR.KLineData.length);
         	 if((GLOBAL_VAR.KLineData.length -json.datas.data.length)>json.datas.data.length){
         		for(var i=1;i<json.datas.data.length;i++){
            		 Array.prototype.shift.apply(GLOBAL_VAR.KLineData);
            	 }
         	 }
         	 
         	 //console.log("后2："+GLOBAL_VAR.KLineData.length);
          }
     }else{
    	 Array.prototype.push.apply(GLOBAL_VAR.KLineData,json.datas.data);
     }*/
...
function getklinedataWebsocket(){
		...
		if(initFristKline){
			...
			if(oldChannel!=channel ){//发送新请求频道，先删除旧渠道
				if(oldChannel!=null){
                    if(top.ExxWebSocket.openWebSocket) {
                        top.ExxWebSocket.websocket.send('{"event":"removeChannel","channel":"' + oldChannel + '"}');
                    }
				...
				var lastTime  = GLOBAL_VAR.chartMgr.getLastTime('BTC123'+'.'+GLOBAL_VAR.market_from+'.'+GLOBAL_VAR.time_type);
...
function getklinedataAjax(){
    ...
    GLOBAL_VAR.G_HTTP_REQUEST = $.getJSON(GLOBAL_VAR.url+"?jsoncallback=?&"+GLOBAL_VAR.requestParam,function(json) {
        if (GLOBAL_VAR.G_HTTP_REQUEST) {
            ...
            if(!json || !json.datas.data || json.datas.data.length==0){
                $(".bk-animationload").fadeOut();
                return;
            }
            if(!json.isSuc){
...
function readCookie() {
    ...
    var market_from=tmp.charts.market_from;
    if(!GLOBAL_VAR.init){
    	market_from=kline.symbol;
    	GLOBAL_VAR.init=true;
    }
    GLOBAL_VAR.market_from=market_from;
    switch_market_selected(market_from);
    ...
    if (tmp.charts.indicsStatus == 'off') {
        switch_indic('off');
    } else if (tmp.charts.indicsStatus == 'on') {
        switch_indic('on');
    }
    if (tmp.charts.toolStatus == 'off') {
        switch_tools('off');
    } else if (tmp.charts.toolStatus == 'on') {
        switch_tools('on');
    }
...
var main = function() {
    window._setMarketFrom = function(content) {
        ...
        refreshTemplate();
        readCookie();
        ChartManager.getInstance().getChart().setMarketFrom(content);
    };
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    var str = "needTickers=1&market=" + mark_from + "&type=" + time_type + "&note=mainkline&assist=" + (kline.money || kline.symbol.split('_')[1]);
    if (limit != null)
        str += "&size=" + limit;
    else
        str += "&since=" + since;
...
function chart_switch_money(money) {
    var tmp = ChartSettings.get();
    tmp.money = money;
    ChartSettings.save();
    //切换辅助货币之前移除旧频道
...
function chart_set_money(money) {
    var tmp = ChartSettings.get();
    tmp.money = money;
    ChartSettings.save();
    //切换辅助货币之前移除旧频道
...
function on_size() {
    var width = window.innerWidth-$("#trade_container:visible").innerWidth();
    ...
    toolBarRect.y = 0;
    toolBarRect.w = width;
    toolBarRect.h = toolBar.is(":visible") ? parseInt(toolBar.css("height")) : 0;
    var toolPanelRect = {};
    toolPanelRect.x = 0;
    toolPanelRect.y = toolBarRect.h;
    ...
    tabBarRect.y = height - (tabBarRect.h + parseInt(tabBar.css("border-top-width")));
    var canvasGroupRect = {};
    ...
    toolBar.css({
        ...
        top:    toolBar.is(":visible") ? toolBarRect.y + 'px' : 0,
        width:  toolBarRect.w + 'px',
        height: toolBar.is(":visible") ? toolBarRect.h + 'px' : 0
    });
    if (toolPanelShown) {
        toolPanel.css({
    ...
    canvasGroup.css({
        ...
        top:    toolBar.is(":visible") ? canvasGroupRect.y + 'px' : 0,
...
function switch_market_selected(id) {
    kline.reset(id);
	$(".market_chooser ul a").removeClass("selected");
	$(".market_chooser ul a[name='"+id+"']").addClass("selected");
	ChartManager.getInstance().getChart()._market_from = id;
	var settings = ChartSettings.get();
    settings.charts.market_from = id;
    ChartSettings.save();
}

function switch_market(id){
	switch_market_selected(id);
	var settings = ChartSettings.get();
    ...
    ChartManager.getInstance().getChart().setMarketFrom(id);
}
...
function setChartMainIndicator(name){
    var tmp = ChartSettings.get();
...
function setChartMainStyle(style) {
    var tmp = ChartSettings.get();
    tmp.charts.chartStyle = style;
    ChartSettings.save();
    var mgr = ChartManager.getInstance();
    mgr.setChartStyle("frame0.k0", style);
    mgr.redraw();
}

function showChartSetting() {
